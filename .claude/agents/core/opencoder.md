...
### Game Development Specialists (Tant-R Override)
- `Game_Design_Agent` - Validate ALL design decisions against DESIGN.MD (ABSOLUTE AUTHORITY)
- `Lead_Developer_Agent` - Godot 4.x / GDScript implementation specialist
- `QA_Agent` - Game testing, performance profiling, device testing
- `Audio_Agent` - SFX, music, AudioManager implementation
- `VFX_Shader_Agent` - Shaders, particles, post-processing effects
- `Asset_Pipeline_Agent` - Blender→Godot pipeline, model/texture optimization
- `Narrative_Level_Agent` - Story structure, level design, progression
